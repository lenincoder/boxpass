...
from path import check_file, new_filedb
...
def logindbmaster(usermaster, passmaster):
    ...
    if check_file():  # comprobar si existe la base de datos
        midb = check_file()  # devuelve una cadena con la ruta de la base de datos
...
def nuevadb(usermaster, passmaster):
    ...
    mi_nuevadb = new_filedb()
    conexion = sqlite3.connect(mi_nuevadb)
    conexion.commit()
    cursor = conexion.cursor()
